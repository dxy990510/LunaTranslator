...
from utils.utils import checkifnewgame
...
class dialog_savedgame_new(QDialog): 
        ...
        def __init__(self, object ) -> None:
                ...
                self.savebutton=[] 
                self.simplebutton("开始游戏",True,lambda: self.startgame(self.currentfocuspath ))
                self.simplebutton("游戏设置",True,self.showsettingdialog) 
                self.simplebutton("删除游戏",True,self.clicked2)
                self.simplebutton("打开目录",True,self.clicked4)
 
                self.simplebutton("添加游戏",False,self.clicked3)
        ...
        def simplebutton(self,text,save,callback):
                button5=QPushButton( )
                button5.setText(_TR(text))
                if save:
                        self.savebutton.append(button5) 
        ...
        def itemfocuschanged(self,b,k):
                ...
                for _btn in self.savebutton:
                        _btn.setEnabled(self.activategamenum>0)  
        def newline(self,k): 
                checkifnewgame(k)
                _pix=QPixmap(savehook_new_data[k]['imagepath'])
                if _pix.isNull():
                      _pix=getExeIcon(k,False)
                gameitem=ItemWidget(_pix,savehook_new_data[k]['title'])
