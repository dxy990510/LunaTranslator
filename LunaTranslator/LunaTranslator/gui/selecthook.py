...
class hookselect(closeashidewindow):
    ...
    getfoundhooksignal=pyqtSignal(list)
    ...
    def findhookchecked(self):  
            if  self.object.textsource: 
                ...
                self.getnewsentence(_TR('！未选定进程！'))
    def getfoundhook(self,lst):
        newhc=[]
        changed={}
        for hookcode,string in lst:
            if hookcode not in self.allres:
                self.allres[hookcode]=[]
                newhc.append(hookcode)
            self.allres[hookcode].append(string)
            changed[hookcode]=string
         
        

        searchtext=self.searchtext2.text() 
          
        for hookcode in changed:
            string=changed[hookcode] 
            if hookcode in newhc:
