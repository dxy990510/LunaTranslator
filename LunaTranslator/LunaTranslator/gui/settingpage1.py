...
def gethookgrid(self) :
        ...
        grids=[
                ...
                [('刷新延迟(ms)',5),(getspinbox(1,10000,globalconfig,'textthreaddelay',callback=lambda x:self.object.textsource.setdelay()),3)],
...
def gethookembedgrid(self) :  
        ...
        grids=[
                 ...
                 [''],
                [('保留原文',5),(getsimpleswitch( globalconfig['embedded'],'keeprawtext',callback=lambda x:self.object.textsource.sendSetting('embeddedScenarioTextVisible',x ))  ,1) ],
                 
                [('翻译等待时间(s)',5),'',(getspinbox(0,30,globalconfig['embedded'],'timeout_translate',double=True,step=0.1,callback=lambda x:self.object.textsource.sendSetting('embeddedTranslationWaitTime',int(x*1000))),3) ],
                ...
                [('修改游戏字体',5),(getsimpleswitch( globalconfig['embedded'] ,'changefont',callback=lambda x:self.object.textsource.sendSetting('embeddedFontFamily',globalconfig['embedded']['changefont_font'] if x else '')),1), (self.gamefont_comboBox,5) ],
                [('修改字体字符集',5),(getsimpleswitch( globalconfig['embedded'] ,'changecharset',callback=lambda x:self.object.textsource.sendSetting('embeddedFontCharSetEnabled',x)),1) ,(getsimplecombobox(_TRL(static_data["charsetmapshow"]),globalconfig['embedded'],'changecharset_charset',callback=lambda x:self.object.textsource.sendSetting('embeddedFontCharSet',static_data["charsetmap"][x])),5)],
...
def setTabOne_direct(self) :  
        ...
        self.tab1grids=[
                ...
                [
                        ('剪贴板',3),(getsimpleswitch(globalconfig['sourcestatus']['copy'],'use',name='copy',parent=self,callback= functools.partial(yuitsu_switch,self, globalconfig['sourcestatus'],'sourceswitchs','copy',self.object.starttextsource),pair='sourceswitchs'),1),'',
                        ('OCR',3),(getsimpleswitch(globalconfig['sourcestatus']['ocr'],'use',name='ocr',parent=self,callback= functools.partial(yuitsu_switch,self,globalconfig['sourcestatus'],'sourceswitchs','ocr',self.object.starttextsource),pair='sourceswitchs'),1),'',
                        ('',4) 
                       
                ],
                [   
                     ('HOOK',3),(getsimpleswitch(globalconfig['sourcestatus']['texthook'],'use',name='texthook',parent=self,callback= functools.partial(yuitsu_switch,self,globalconfig['sourcestatus'],'sourceswitchs','texthook',self.object.starttextsource),pair='sourceswitchs'),1),'',
                    ('HOOK_内嵌',3),(getsimpleswitch(globalconfig['sourcestatus']['embedded'],'use',name='embedded',parent=self,callback= functools.partial(yuitsu_switch,self,globalconfig['sourcestatus'],'sourceswitchs','embedded',self.object.starttextsource),pair='sourceswitchs'),1),'',
                      
                ], 
        ]  

        (getcolorbutton(globalconfig ,'',name='selectbutton',parent=self,icon='fa.gear',constcolor="#FF69B4",callback=lambda :self.object.AttachProcessDialog.showsignal.emit()),1)
        (getcolorbutton(globalconfig ,'',name='selectbuttonembed',parent=self,icon='fa.gear',constcolor="#FF69B4",callback=lambda :self.object.AttachProcessDialog.showsignal.emit()),1)
        (getcolorbutton(globalconfig ,'',name='selecthookbutton',parent=self,icon='fa.gear',constcolor="#FF69B4",callback=lambda  : self.object.hookselectdialog.showsignal.emit() ),1)
